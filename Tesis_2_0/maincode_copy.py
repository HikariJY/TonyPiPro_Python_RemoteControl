...
try_flag_color, change_flag_color, open_flag_color, record_flag_color = False, False, False, False
try_flag, start_flag, change_flag, open_flag, record_flag = False, False, False, False, False
fps_list = []
prev_time = 0
detector1 = PoseDetector(detectionCon=0.955, trackCon=0.955)
detector2 = PoseDetector(detectionCon=0.955, trackCon=0.955)
...
def Capture():
    global try_flag, start_flag, try_flag_color, capR, capL, id_flag, prev_time, fps_list
    if try_flag or start_flag:
        ...
        if ret_R:
            ...
            if ret_L:
                ...
                frame_R = cv2.resize(frame_R, frame_size, interpolation=cv2.INTER_AREA)
                frame_L = cv2.resize(frame_L, frame_size, interpolation=cv2.INTER_AREA)
                if pose_flag:
                    frame_R = detector1.findPose(frame_R, draw=True)
                    lmList_R, bboxInfo_R = detector1.findPosition(frame_R, draw=False, bboxWithHands=False)
                    frame_L = detector2.findPose(frame_L, draw=True)
                    lmList_L, bboxInfo_L = detector2.findPosition(frame_L, draw=False, bboxWithHands=False)
            ...
            cmbbx_right.configure(state=tk.NORMAL)
            cmbbx_left.configure(state=tk.NORMAL)
def ChangeEvent(): print('CahngeEvent')
def OpenEvent(): print('OpenEvent')
def RecordEvent(): print ('RecordEvent')
